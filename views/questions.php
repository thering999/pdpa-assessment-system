    ...
    <form method="post" action="?a=save_answers" onsubmit="this.querySelector('button[type=submit]').disabled=true;">
            ...
            <div class="excel-tabs">
                <?php $first = true; foreach ($byCat as $cid => $cat): ?>
                    <button type="button" class="excel-tab <?= $first ? 'active' : '' ?>" onclick="switchTab(<?= (int)$cid ?>, this)">
                    ...
                    <div id="tab-<?= (int)$cid ?>" class="tab-panel <?= $first ? 'active' : '' ?>">
                        ...
                        <?php foreach ($cat['questions'] as $q): $qid=(int)$q['id']; $sel=(int)($answers[$qid] ?? 0); ?>
                            <div class="q-item">
                                ...
                                <div class="q-notes" style="margin-top:10px;">
                                    <label>หมายเหตุ/เหตุผล:
                                        <input type="text" class="note-input" data-qid="<?= $qid ?>" name="notes<?= $qid ?>" value="<?= htmlspecialchars($notes[$qid] ?? '') ?>" style="width:100%;padding:6px;border-radius:5px;border:1px solid #bbb;" maxlength="500" />
                                    </label>
                                </div>
                            </div>
                        <?php endforeach; ?>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.score-option').forEach(option => {
                option.addEventListener('click', function(e) {
                    ...
                    }).then(() => { showSaveFeedback(); })
                      .catch(() => { console.log('Save failed, will retry on form submit'); });
                });
            });
            ...
            document.querySelectorAll('.note-input').forEach(inp => {
                const handler = () => {
                    ...
                    const note = encodeURIComponent(inp.value.trim());
                    fetch('?a=save_answer', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                        body: `qid=${qid}&note=${note}`
                    }).then(() => { showSaveFeedback(); })
                      .catch(() => { /* ignore, will submit with form */ });
                };
                inp.addEventListener('input', () => {
                    const id = inp.dataset.qid;
                    clearTimeout(noteTimers[id]);
                    noteTimers[id] = setTimeout(handler, 600);
                });
